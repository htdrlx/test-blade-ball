...
function Library:create_ui()
    ...
    function self:set_gui_visibility(state: boolean)
        ...
        -- Keep the ScreenGui enabled so a restore control can remain available.
...
-- ============================================================
-- Zuro✿ LOCAL VISUAL / PERFORMANCE EXTENSION
-- Does not modify the existing gameplay automation logic.
-- ============================================================
do
    local ZRun = cloneref(game:GetService("RunService"))
    local ZPlayers = cloneref(game:GetService("Players"))
    local ZDebris = cloneref(game:GetService("Debris"))

    local LP = ZPlayers.LocalPlayer
    local Camera = workspace.CurrentCamera
    ...
    local function isFX(x)
        return x:IsA("ParticleEmitter")
            or x:IsA("Trail")
            or x:IsA("Beam")
            or x:IsA("Smoke")
            or x:IsA("Fire")
    ...
    local function remember(x)
        if not isFX(x) or originals[x] then return end
        originals[x] = {
            enabled = x.Enabled,
            rate = x:IsA("ParticleEmitter") and x.Rate or nil,
        }
        tracked[x] = true
    end
    ...
    local function restore(x)
        ...
        if x.Parent then
            pcall(function()
                x.Enabled = o.enabled
                if o.rate and x:IsA("ParticleEmitter") then
                    x.Rate = o.rate
                end
            end)
        end
    ...
    local function applySmartFX()
        for x in pairs(tracked) do
            ...
            local visible = screen.Z > 0
                and screen.X >= 0 and screen.Y >= 0
                and screen.X <= Camera.ViewportSize.X
                and screen.Y <= Camera.ViewportSize.Y

            local maxDistance = 180 * (0.5 + state.quality)

            if not visible or distance > maxDistance then
                pcall(function() x.Enabled = false end)
            elseif state.dynamicFX and x:IsA("ParticleEmitter") then
                local o = originals[x]
                if o and o.rate then
                    local distanceFactor = math.clamp(1 - distance / maxDistance, 0.2, 1)
                    local fpsFactor = math.clamp(currentFPS / 60, 0.4, 1)
                    pcall(function()
                        x.Enabled = o.enabled
                        x.Rate = o.rate * state.quality * distanceFactor * fpsFactor
                    end)
                end
            end
        end
    ...
    fpsLabel.Position = UDim2.new(1, -102, 0, 10)
    fpsLabel.BackgroundColor3 = Color3.fromRGB(25, 25, 30)
    fpsLabel.BackgroundTransparency = 0.18
    ...
    -- Ball speed ring (local visual)
    local ringGui = Instance.new("BillboardGui")
    ringGui.Name = "ZuroBallSpeedRing"
    ringGui.Size = UDim2.fromOffset(120, 120)
    ringGui.AlwaysOnTop = true
    ringGui.Enabled = false
    ringGui.Parent = LP:WaitForChild("PlayerGui")
    ...
    -- Persistent restore button.
    task.defer(function()
        local ui = Library and Library._ui
        if not ui then return end

        local restore = Instance.new("TextButton")
        restore.Name = "ZuroRestore"
        restore.Size = UDim2.fromOffset(82, 30)
        restore.Position = UDim2.new(0, 12, 0.5, -15)
        restore.BackgroundColor3 = Color3.fromRGB(24, 24, 29)
        restore.TextColor3 = Color3.fromRGB(245, 245, 248)
        restore.Text = "Zuro✿"
        restore.TextSize = 12
        ...
        local function sync()
            restore.Visible = not library._ui_open
        end

        restore.Activated:Connect(function()
            library._ui_open = true
            pcall(function() library:set_gui_visibility(true) end)
            sync()
        end)
        ...
        getgenv()._Zuro_RestoreUI = function()
            library._ui_open = true
            pcall(function() library:set_gui_visibility(true) end)
            sync()
        end
    ...
    local fxAdded = workspace.DescendantAdded:Connect(function(x)
        if isFX(x) then
            task.defer(function() remember(x) end)
        end
    end)

    local render = ZRun.RenderStepped:Connect(function(dt)
        fpsFrames += 1
        if os.clock() - fpsClock >= 0.5 then
            currentFPS = math.floor(fpsFrames / (os.clock() - fpsClock) + 0.5)
            fpsFrames = 0
            fpsClock = os.clock()
            if state.fps then
                fpsLabel.Text = ("FPS: %d"):format(currentFPS)
            end
        end

        scanClock += dt
        if scanClock >= 0.4 then
            scanClock = 0
            scanFX()
            applySmartFX()
        end
        ...
        if state.ring then
            ...
            if root and ball then
                ...
                ringGui.Adornee = root
                local radius = math.clamp(55 + speed * 0.35, 55, 135)
                ringGui.Size = UDim2.fromOffset(radius * 2, radius * 2)
                ring.Rotation = (ring.Rotation + math.clamp(speed / 10, 1, 14)) % 360
    ...
    local perf = Visual:create_module({
        title = "Smart FX",
        flag = "Zuro_SmartFX",
        description = "Cull off-screen local visual effects",
        section = "left",
    ...
    perf:create_slider({
        ...
        round_number = false,
        callback = function(v)
            state.quality = math.clamp(v, 0.25, 1)
        end,
    })
    ...
    Visual:create_module({
        title = "FPS Monitor",
        flag = "Zuro_FPS",
        description = "Client FPS counter",
        section = "left",
        callback = function(on)
            state.fps = on
            fpsGui.Enabled = on
    ...
    Visual:create_module({
        ...
        flag = "Zuro_BallRing",
        description = "Local ring scales with ball speed",
        section = "right",
        callback = function(on)
            state.ring = on
        end,
    ...
    Visual:create_module({
        title = "Sword Sound",
        flag = "Zuro_SwordSound",
        description = "Local sound preference for showcase effects",
        section = "right",
        callback = function(on)
            state.swordSound = on
        end,
    ...
    getgenv()._Zuro_VisualCleanup = function()
        pcall(function() fxAdded:Disconnect() end)
        pcall(function() render:Disconnect() end)
        pcall(function() fpsGui:Destroy() end)
        pcall(function() ringGui:Destroy() end)
        for x in pairs(tracked) do
            restore(x)
        end
    end
