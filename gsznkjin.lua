...
function Zenthra:CreateWindow(options)
    ...
    local background = create("ImageLabel", {Name="ZuroBackground", Size=UDim2.fromScale(1,1), BackgroundColor3=Color3.fromRGB(7,14,23), BorderSizePixel=0, Image="rbxassetid://92054253025564", ImageTransparency=0.15, ScaleType=Enum.ScaleType.Crop, ZIndex=0}, screen)
    local backgroundTint = create("Frame", {Name="ZuroBackgroundTint", Size=UDim2.fromScale(1,1), BackgroundColor3=Color3.fromRGB(30,91,132), BackgroundTransparency=0.84, BorderSizePixel=0, ZIndex=0}, screen)
    ...
    for _, handle in ipairs(dragHandles) do
        handle.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                dragging = true; dragStart = input.Position; shellStart = shell.Position
            end
        end)
    end
...
local ZP_HttpService = game:GetService("HttpService")

_G.Zuro_demo_config = _G.Zuro_demo_config or {}
local ZCFG = _G.Zuro_demo_config
ZCFG.saved = ZCFG.saved or {}

local ZP_FOLDER = "Zuro"
local ZP_SUBFOLDER = "Zuro/cfg"
local ZP_PATH = "Zuro/cfg/ui_demo.json"

local ZP_can_write = false
pcall(function()
    if type(isfolder) == "function" and type(makefolder) == "function"
       ...
       and type(isfile) == "function" then
        if not isfolder(ZP_FOLDER) then makefolder(ZP_FOLDER) end
        if not isfolder(ZP_SUBFOLDER) then makefolder(ZP_SUBFOLDER) end
        ZP_can_write = true
    end
end)

local ZP_ready = false
local ZP_dirty = false
local ZP_fresh = true

local function ZP_save()
    if not ZP_can_write then return end
    pcall(function()
        writefile(ZP_PATH, ZP_HttpService:JSONEncode(ZCFG.saved))
    end)
end

local function ZP_load()
    if not ZP_can_write then return end
    pcall(function()
        if isfile(ZP_PATH) then
            local ok, decoded = pcall(function()
                return ZP_HttpService:JSONDecode(readfile(ZP_PATH))
            end)
...
local ZP_used = {}
local function ZP_key(prefix, name)
    local base = prefix ~= "" and (prefix .. "_" .. ZP_slug(name)) or ZP_slug(name)
    if base == "" then base = "widget" end
    local key, n = base, 1
    while ZP_used[key] do
        n = n + 1
        key = base .. "_" .. n
    end
    ZP_used[key] = true
...
local function ZP_mark()
    ZP_dirty = true
end

task.spawn(function()
    while task.wait(1) do
        if ZP_ready and ZP_dirty then
            ZP_dirty = false
            ZP_save()
        end
    end
...
local function ZP_persist(section, prefix)
    local raw = {
    ...
    function section:AddToggle(data)
        data = data or {}
        local key = ZP_key(prefix, data.Name)
        local stored = ZCFG.saved[key]
        local base = ZP_off(data.Default == true) == true
        ...
        local user = data.Callback
        data.Callback = function(value)
            ZCFG.saved[key] = value == true
            ZP_mark()
            if user then user(value) end
        end
        local control = raw.AddToggle(self, data)
        ZP_restore[#ZP_restore + 1] = function()
            local v = ZCFG.saved[key]
            if type(v) ~= "boolean" then return end
            if v ~= (control.Get() == true) then
                control.Set(v, false)
            elseif v ~= base and user then
                task.spawn(user, v)
            end
        end
        local wrapped = { Instance = control.Instance, Get = control.Get, _key = key }
        function wrapped.Set(value, silent)
            local st = value == true
            control.Set(st, silent)
            ZCFG.saved[key] = st
            ZP_mark()
        end
        wrapped.change_state = function(_, st) return wrapped.Set(st) end
        return wrapped
    end
    ...
    function section:AddSlider(data)
        data = data or {}
        local key = ZP_key(prefix, data.Name)
        local stored = tonumber(ZCFG.saved[key])
        local base = data.Default
        if stored then
            data.Default = stored
        else
            ZCFG.saved[key] = data.Default
        end
        local user = data.Callback
        data.Callback = function(value)
            ZCFG.saved[key] = value
            ZP_mark()
            if user then user(value) end
        end
        local obj = raw.AddSlider(self, data)
        ZP_restore[#ZP_restore + 1] = function()
            local v = tonumber(ZCFG.saved[key])
            if not v then return end
            if v ~= obj.Get() then obj:Set(v, true) end
            if v ~= base and user then task.spawn(user, v) end
        end
        local rawSet = obj.Set
        function obj:Set(value, silent)
            rawSet(self, value, silent)
            ZCFG.saved[key] = self.Get()
            ZP_mark()
        end
        obj._key = key
        return obj
    ...
    function section:AddDropdown(data)
        ...
        local obj = raw.AddDropdown(self, data)
        ZP_restore[#ZP_restore + 1] = function()
            local v = ZCFG.saved[key]
        ...
        obj.update = function(_, v) return obj:Set(v) end
        obj._key = key
        return obj
    end
    ...
    function section:AddInput(data)
        ...
        if type(stored) == "string" then
            ...
            ZCFG.saved[key] = data.Default or ""
        end
        local user = data.Callback
        data.Callback = function(value)
            ZCFG.saved[key] = value
        ...
        local obj = raw.AddInput(self, data)
        ZP_restore[#ZP_restore + 1] = function()
            local v = ZCFG.saved[key]
            if type(v) ~= "string" or v == "" then return end
            if v ~= obj.Get() then obj:Set(v) end
            if user then task.spawn(user, v) end
        end
        local rawSet = obj.Set
        function obj:Set(value)
            rawSet(self, value)
            ZCFG.saved[key] = tostring(value)
            ZP_mark()
        end
        obj.update = function(_, v) return obj:Set(v) end
        obj._key = key
...
local function ZP_wrap_tab(tab)
    local rawAddSection = tab.AddSection
    function tab:AddSection(config)
        config = config or {}
        local key = ZP_key("", config.Title)
        local stored = ZCFG.saved[key]
        local base = ZP_off(config.Enabled == true) == true
        if type(stored) == "boolean" then
        ...
        local user = config.Callback
        config.Callback = function(state)
            ZCFG.saved[key] = state == true
            ZP_mark()
            if user then user(state) end
        end
        local section = rawAddSection(self, config)
        ZP_restore[#ZP_restore + 1] = function()
            local v = ZCFG.saved[key]
            if type(v) ~= "boolean" then return end
            if v ~= (section:GetEnabled() == true) then
                section:SetEnabled(v)
            elseif v ~= base and user then
                task.spawn(user, v)
            end
        end
        local rawSetEnabled = section.SetEnabled
        function section:SetEnabled(state, silent)
            rawSetEnabled(self, state, silent)
            ZCFG.saved[key] = state == true
            ZP_mark()
        end
        function section:change_state(state) return section:SetEnabled(state) end
        return ZP_persist(section, key)
    end
    return tab
end

local function ZP_wrap_window(window)
    local rawAddTab = window.AddTab
    function window:AddTab(name, icon)
        return ZP_wrap_tab(rawAddTab(self, name, icon))
    end
    return window
end

_G.Zuro_demo_state = {
    can_write = function() return ZP_can_write end,
    path = ZP_PATH,
    fresh = function() return ZP_fresh end,
    saved = function() return ZCFG.saved end,
    count = function()
        local n = 0
        for _ in pairs(ZCFG.saved) do n = n + 1 end
        return n
    end,
    enabled_count = function()
        local n = 0
        for _, v in pairs(ZCFG.saved) do if v == true then n = n + 1 end end
        return n
    end,
    save_now = function() ZP_save() end,
}

local function ZP_apply()
    for _, fn in ipairs(ZP_restore) do
        pcall(fn)
    end
    ZP_ready = true
    ZP_dirty = false
...
local Window = ZP_wrap_window(Zenthra:CreateWindow({
    Title = "Zuro",
    Subtitle = "ui demo",
}))

local MainTab    = Window:AddTab("main", "grid")
local InputsTab  = Window:AddTab("inputs", "sliders")
local StateTab   = Window:AddTab("state", "eye")

local basics = MainTab:AddSection({
    Column = "Left",
    Title = "Combat Assist",
    Description = "Core controls and quick actions",
    Enabled = false,
    Callback = function(state)
        print("[section] Basics ->", state)
    end,
})
...
basics:AddToggle({
    ...
    Default = false,
    Callback = function(v) print("[toggle] Example ->", v) end,
})

basics:AddToggle({
    Name = "Ball Warning",
    Default = false,
    Callback = function(v) print("[toggle] Second ->", v) end,
})

basics:AddButton({
    Name = "Notify Me",
    Callback = function()
        Window:Notify({ Title = "Zuro", Content = "Button pressed", Duration = 3 })
    end,
})

local folded = MainTab:AddSection({
    Column = "Right",
    Title = "Collapsed Section",
    Description = "Starts disabled",
    Enabled = false,
    Callback = function(state)
        print("[section] Collapsed ->", state)
    end,
})

folded:AddToggle({ Name = "Hidden Option", Default = false })
folded:AddButton({ Name = "Hidden Action" })

local values = InputsTab:AddSection({
    Column = "Left",
    Title = "Timing & Range",
...
local accuracySlider = values:AddSlider({
    Name = "Accuracy",
    Min = 1, Max = 100, Default = 50,
    Callback = function(v) print("[slider] Accuracy ->", v) end,
})

local thresholdSlider = values:AddSlider({
    Name = "Threshold",
    Min = 0, Max = 10, Default = 3,
    Callback = function(v) print("[slider] Threshold ->", v) end,
})

values:AddRangeSlider({
    Name = "Min - Max",
    Min = 1, Max = 100, Low = 25, High = 75,
    Callback = function(lo, hi) print("[range] ->", lo, hi) end,
})

local picks = InputsTab:AddSection({
    Column = "Right",
    Title = "Display Options",
    Description = "Selection and typing",
    Enabled = false,
})

local modeDropdown = picks:AddDropdown({
    Name = "Parry Mode",
    Default = "Remote",
    Options = {"Remote", "Keypress"},
    Callback = function(v) print("[dropdown] Mode ->", v) end,
})

local curveDropdown = picks:AddDropdown({
    Name = "Curve",
    Default = "Camera",
    Options = {"Camera", "Backwards", "Dot", "Slow", "High", "Left", "Right"},
    Callback = function(v) print("[dropdown] Curve ->", v) end,
})

local nameInput = picks:AddInput({
    Name = "Username",
    Placeholder = "Enter username...",
    Default = "",
    Callback = function(v) print("[input] Username ->", v) end,
})

local api = StateTab:AddSection({
    Column = "Left",
    Title = "Programmatic Control",
    Description = "Set values from code",
    Enabled = false,
})

local watched = api:AddToggle({ Name = "Controlled Toggle", Default = false })

api:AddButton({
    Name = "Toggle it ON",
    Callback = function()
        watched:Set(true)
        print("[api] toggle Get() =", watched.Get())
    end,
})

api:AddButton({
    Name = "Toggle it OFF (change_state)",
    Callback = function()
        watched:change_state(false)
        print("[api] toggle Get() =", watched.Get())
    end,
})

api:AddButton({
    Name = "Set dropdown to Keypress",
    Callback = function()
        modeDropdown:Set("Keypress")
        print("[api] dropdown Get() =", modeDropdown.Get())
    end,
})

api:AddButton({
    Name = "Set curve via :update()",
    Callback = function()
        curveDropdown:update("High")
        print("[api] curve Get() =", curveDropdown.Get())
    end,
})
...
api:AddButton({
    ...
    Callback = function()
        ...
        print("[api] input Get() =", nameInput.Get())
    end,
})

local sections = StateTab:AddSection({
    Column = "Right",
...
sections:AddButton({
    Name = "Expand collapsed section",
    Callback = function()
        folded:SetEnabled(true)
        print("[api] folded GetEnabled() =", folded:GetEnabled())
    end,
})

sections:AddButton({
    Name = "Collapse it (change_state)",
    Callback = function()
        folded:change_state(false)
        print("[api] folded GetEnabled() =", folded:GetEnabled())
    end,
})
...
local persist = StateTab:AddSection({
    Column = "Right",
    Title = "Persistence",
    Description = "Auto save and auto load",
    Enabled = false,
})

persist:AddButton({
    Name = "Show saved key count",
    Callback = function()
        local n = 0
        for _ in pairs(ZCFG.saved) do n = n + 1 end
        Window:Notify({ Title = "persistence", Content = n .. " keys tracked", Duration = 4 })
        print("[persist] tracking", n, "keys ->", ZP_PATH)
    end,
})
...
persist:AddButton({
    ...
    Callback = function()
        ...
        Window:Notify({
            Title = "persistence",
            Content = ZP_can_write and "written to disk" or "no file access",
            Duration = 4,
        })
    end,
})
...
persist:AddButton({
    Name = "Print saved values",
    Callback = function()
        print("[persist] ---- saved config ----")
        for k, v in pairs(ZCFG.saved) do print("   ", k, "=", tostring(v)) end
    end,
})
...
task.spawn(function()
    ...
    Window:Notify({
        Title = "Zuro",
        Content = ZP_can_write and "config loaded" or "no file access - session only",
        Duration = 4,
    })
end)
