...
--[[
    Zuro✿ Visual / Performance Pack
    Safe local-only additions:
      • Sword FX / local skin-style preview
      • Sword sound toggle (controls local sounds on equipped sword)
      • Emote preview scanner (local animations already present in the game)
      • Ball Speed Ring with smart culling
      • Lightweight FPS monitor
      • Visual Lite / Balanced / Ultra quality

    Does not alter the existing gameplay automation/parry code.
]]
...
do
    ...
    local qualityCfg = {
        Lite = { particle = 0.35, maxDistance = 55 },
        Balanced = { particle = 0.65, maxDistance = 90 },
        Ultra = { particle = 1.00, maxDistance = 150 },
    }

    local function getCfg()
        return qualityCfg[ZuroVisual.quality] or qualityCfg.Balanced
    end
    ...
    local function getEquippedSwordModel()
        local char = getChar()
        if not char then return nil end
        local best
        for _, obj in ipairs(char:GetChildren()) do
            if obj:IsA("Model") then
                ...
                if n:find("sword") or n:find("blade") or n:find("katana") then
                    best = obj
                    break
                end
            end
    ...
    local function cleanupSwordFX()
        if ZuroVisual.swordConn then
            ZuroVisual.swordConn:Disconnect()
            ZuroVisual.swordConn = nil
        end
        if ZuroVisual.swordHighlight then
            pcall(function() ZuroVisual.swordHighlight:Destroy() end)
        end
        if ZuroVisual.swordTrail then
            pcall(function() ZuroVisual.swordTrail:Destroy() end)
        end
        ZuroVisual.swordHighlight = nil
        ZuroVisual.swordTrail = nil
    end

    local function applySwordFX()
        cleanupSwordFX()
        if not ZuroVisual.effects then return end
        local sword = getEquippedSwordModel()
        if not sword then return end

        local highlight = Instance.new("Highlight")
        highlight.Name = "ZuroLocalSwordAura"
        highlight.FillTransparency = (ZuroVisual.quality == "Ultra") and 0.55 or 0.72
        highlight.OutlineTransparency = 0.12
        highlight.Parent = sword
        ZuroVisual.swordHighlight = highlight

        local primary = sword.PrimaryPart or sword:FindFirstChildWhichIsA("BasePart", true)
        if primary and ZuroVisual.quality ~= "Lite" then
            ...
            a1.Name = "ZuroTrailA1"
            a1.Position = Vector3.new(0, -0.55, 0)
            a1.Parent = primary

            local trail = Instance.new("Trail")
            trail.Name = "ZuroLocalSwordTrail"
            trail.Attachment0 = a0
            trail.Attachment1 = a1
            trail.Lifetime = (ZuroVisual.quality == "Ultra") and 0.18 or 0.10
            trail.MinLength = 0.08
            trail.Transparency = NumberSequence.new(0.25, 1)
            trail.Parent = primary
            ZuroVisual.swordTrail = trail
        end
    end

    local function setSwordSounds(on)
        local char = getChar()
        if not char then return end
        for _, obj in ipairs(char:GetDescendants()) do
            if obj:IsA("Sound") then
                local n = obj.Name:lower()
                if n:find("sword") or n:find("swing") or n:find("slash") or n:find("hit") then
                    obj.Volume = on and (obj:GetAttribute("ZuroOriginalVolume") or obj.Volume) or 0
                end
            end
        end
    end
    ...
    local function getBallSpeed()
        ...
        if not balls then return 0, nil end
        local best, bestDist
        local char = getChar()
        ...
        for _, ball in ipairs(balls) do
            if ball:IsA("BasePart") then
                ...
                local d = origin and (ball.Position - origin).Magnitude or math.huge
        ...
        return best.AssemblyLinearVelocity.Magnitude, best
    end

    local function destroyRing()
        if ZuroVisual.ringPart then
            pcall(function() ZuroVisual.ringPart:Destroy() end)
        end
        ZuroVisual.ringPart = nil
    end

    local function createRing()
        if ZuroVisual.ringPart then return end
        local p = Instance.new("Part")
        p.Name = "ZuroBallSpeedRing"
        ...
        p.Transparency = 0.35
        p.Size = Vector3.new(0.12, 8, 8)
        p.Parent = workspace
        ZuroVisual.ringPart = p
    end

    local function setupFPSGui()
        if ZuroVisual.fpsGui then return end
        local gui = Instance.new("ScreenGui")
        gui.Name = "ZuroFPSMonitor"
        ...
        label.Position = UDim2.new(1, -100, 0, 8)
        ...
        ZuroVisual.fpsGui = gui
        ZuroVisual.fpsLabel = label
    end

    local function destroyFPSGui()
        if ZuroVisual.fpsGui then
            pcall(function() ZuroVisual.fpsGui:Destroy() end)
        end
        ZuroVisual.fpsGui = nil
        ZuroVisual.fpsLabel = nil
    end

    local function playLocalEmoteByName(name)
        local char = getChar()
        local humanoid = char and char:FindFirstChildOfClass("Humanoid")
        local animator = humanoid and humanoid:FindFirstChildOfClass("Animator")
        if not animator then return false end

        if ZuroVisual.emoteTrack then
            pcall(function() ZuroVisual.emoteTrack:Stop(0.1) end)
            ZuroVisual.emoteTrack = nil
        end

        local found
        local rs = ReplicatedStorage
        for _, obj in ipairs(rs:GetDescendants()) do
            if obj:IsA("Animation") and obj.Name:lower() == name:lower() then
                found = obj
                break
            end
        end
        ...
        local ok, track = pcall(function() return animator:LoadAnimation(found) end)
        if not ok or not track then return false end
        ZuroVisual.emoteTrack = track
        track:Play(0.12, 1, 1)
        return true
    end

    Visual:create_module({
        title = "Sword Showcase",
        flag = "Zuro_Sword_Showcase",
        description = "Local sword aura and trail preview",
        section = "right",
        callback = function(state)
            ZuroVisual.effects = state
            if state then
                applySwordFX()
            else
                cleanupSwordFX()
    ...
    Visual:create_module({
        title = "Sword Sounds",
        flag = "Zuro_Sword_Sounds",
        description = "Toggle local sword sounds",
        section = "right",
        callback = function(state)
            ZuroVisual.sounds = state
            setSwordSounds(state)
        end,
    })

    Visual:create_module({
        title = "Ball Speed Ring",
        flag = "Zuro_Ball_Speed_Ring",
        description = "Local ring scales with nearest ball speed",
        section = "right",
        callback = function(state)
            ZuroVisual.ring = state
            if state then createRing() else destroyRing() end
        end,
    })

    Visual:create_dropdown({
        title = "Visual Quality",
        flag = "Zuro_Visual_Quality",
        options = {"Lite", "Balanced", "Ultra"},
        multi_dropdown = false,
        maximum_options = 3,
        callback = function(value)
            ZuroVisual.quality = value
            if ZuroVisual.effects then applySwordFX() end
        end,
    })

    Visual:create_module({
        title = "FPS Monitor",
        flag = "Zuro_FPS_Monitor",
        description = "Lightweight local FPS counter",
        section = "right",
        callback = function(state)
            ZuroVisual.fps = state
            if state then setupFPSGui() else destroyFPSGui() end
        end,
    })

    Visual:create_module({
        title = "Emote Preview",
        flag = "Zuro_Emote_Preview",
        description = "Play an existing local emote animation",
        section = "right",
        callback = function(state)
            if not state and ZuroVisual.emoteTrack then
                pcall(function() ZuroVisual.emoteTrack:Stop(0.1) end)
                ZuroVisual.emoteTrack = nil
            end
        end,
    ...
    local lastSword = nil
    local timeAcc = 0
    ZuroVisual.conn = RunService.Heartbeat:Connect(function(dt)
        if not ZuroVisual.enabled then return end
        timeAcc += dt
        if timeAcc < 0.10 then return end
        timeAcc = 0
        ...
        if ZuroVisual.effects then
            local sword = getEquippedSwordModel()
            if sword ~= lastSword then
                lastSword = sword
                applySwordFX()
            end
        end

        if ZuroVisual.ring and ZuroVisual.ringPart and root and cam then
            local speed, ball = getBallSpeed()
            if ball then
                local pos = ball.Position
                local _, onScreen = cam:WorldToViewportPoint(pos)
                local distance = (pos - cam.CFrame.Position).Magnitude
                if onScreen and distance <= cfg.maxDistance then
                    local radius = math.clamp(3.5 + speed * 0.018, 3.5, 16)
                    local spin = math.clamp(0.15 + speed * 0.004, 0.15, 2.2)
                    ZuroVisual.ringPart.Transparency = 0.35
                    ZuroVisual.ringPart.Size = Vector3.new(0.12, radius * 2, radius * 2)
                    ZuroVisual.ringPart.CFrame = CFrame.new(root.Position - Vector3.new(0, 2.7, 0))
                        * CFrame.Angles(0, os.clock() * spin, math.rad(90))
                else
                    ZuroVisual.ringPart.Transparency = 1
                end
            else
                ZuroVisual.ringPart.Transparency = 1
            end
    ...
    ZuroVisual.fpsConn = RunService.RenderStepped:Connect(function(dt)
        if not ZuroVisual.fps or not ZuroVisual.fpsLabel then return end
        fpsElapsed += dt
        fpsFrames += 1
        if fpsElapsed >= 0.5 then
            local fps = math.floor((fpsFrames / fpsElapsed) + 0.5)
            ZuroVisual.fpsLabel.Text = "FPS: " .. tostring(fps)
...
do
	...
	getgenv()._Fallen_AntiLag_Cleanup = disable_anti_lag
end
...
do
	...
	getgenv()._Fallen_TD_Stop = function()
...
do
	...
	getgenv()._Fallen_StaffDet_Stop = _stop_staff_detection
...
local function create_performance_overlay()
    ...
    OverlayGui.Name = 'FallenPerformanceMetrics'
...
local function create_ball_stats_gui()
    ...
    local OverlayGui = Instance.new('ScreenGui')
    OverlayGui.Name = 'FallenBallMetrics'
    OverlayGui.ResetOnSpawn = false
    OverlayGui.IgnoreGuiInset = true
    OverlayGui.DisplayOrder = 99
    OverlayGui.Parent = CoreGui
...
getgenv()._Fallen_ESP_Stop = stop_ability_esp
...
local function cleanup_visual_stats()
    ...
    destroy_ball_stats()
    pcall(function()
        if getgenv()._Fallen_PlayerPanel_Stop then getgenv()._Fallen_PlayerPanel_Stop() end
    end)
end
...
do
    ...
    getgenv()._Fallen_PlayerPanel_Stop = destroy_player_panel
end
...
do
	...
	getgenv()._Fallen_TB_Stop = stop_triggerbot

	Blatant:create_module({
		title = 'Triggerbot',
		flag = 'Triggerbot',
...
local function create_manual_spam_overlay()
	...
	local OverlayGui = Instance.new('ScreenGui');
	OverlayGui.Name = 'FallenManualSpamOverlay';
	OverlayGui.ResetOnSpawn = false;
	OverlayGui.IgnoreGuiInset = true;
...
local function create_cps_counter()
	...
	CounterGui.Name = 'FallenCpsCounter';
...
do
	...
	local function create_curve_overlay()
		...
		local OverlayGui = Instance.new('ScreenGui')
		OverlayGui.Name = 'FallenCurveOverlay'
		OverlayGui.ResetOnSpawn = false
		OverlayGui.IgnoreGuiInset = true
		OverlayGui.DisplayOrder = 100
		OverlayGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
local _last_profile_path = 'Fallen/last_profile.txt'
local _cfg_name = 'default'

    local function _get_profiles()
        local files = {}
        local ok, result = pcall(listfiles, 'Fallen/profiles')
        if ok and result then
    ...
    pcall(function() makefolder('Fallen/profiles') end)

    if isfile(_last_profile_path) then
        local last_name = readfile(_last_profile_path):gsub('%s+', '')
        if last_name ~= '' then
            local path = 'Fallen/profiles/' .. last_name .. '.json'
            if isfile(path) then
    ...
    config_module:create_button({
        ...
        callback = function()
            ...
            local ok = pcall(function()
                writefile('Fallen/profiles/' .. _cfg_name .. '.json', HttpService:JSONEncode(Library._config))
            end)
            if ok then
                local p = _get_profiles()
                table.insert(p, 1, 'Default')
    ...
    config_module:create_button({
        ...
        callback = function()
            ...
            local path = 'Fallen/profiles/' .. _cfg_name .. '.json'
            if not isfile(path) then
...
library:removed(function()
    ...
    pcall(function() if getgenv()._Fallen_TB_Stop then getgenv()._Fallen_TB_Stop() end end)
    pcall(function() if getgenv()._Fallen_ESP_Stop then getgenv()._Fallen_ESP_Stop() end end)
    pcall(function() if getgenv()._Fallen_StaffDet_Stop then getgenv()._Fallen_StaffDet_Stop() end end)
    stop_force_stats()
    if Target_Lock_Highlight then
        Target_Lock_Highlight:Destroy()
        Target_Lock_Highlight = nil
    ...
    pcall(function()
        if getgenv()._Fallen_AntiLag_Cleanup then
            getgenv()._Fallen_AntiLag_Cleanup()
        end
    end)
    if Connections_Manager["Auto Parry"] then
        Connections_Manager["Auto Parry"]:Disconnect()
    ...
    kill_preclick_stop()
    pcall(function()
        if getgenv()._Fallen_TD_Stop then getgenv()._Fallen_TD_Stop() end
    end)
end)
