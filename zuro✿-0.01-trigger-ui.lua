...
pcall(function()
    if getgenv()._Zuro✿_Cleanup then
        getgenv()._Zuro✿_Cleanup()
        getgenv()._Zuro✿_Cleanup = nil
    end
end)
...
if not NotificationHost then
    ...
    NotificationHost.Name = "Zuro✿Notifications"
...
do
	...
	getgenv()._Zuro✿_AntiLag_Cleanup = disable_anti_lag
end
...
do
	...
	getgenv()._Zuro✿_TD_Stop = function()
...
do
	...
	getgenv()._Zuro✿_StaffDet_Stop = _stop_staff_detection
...
local function create_performance_overlay()
    ...
    OverlayGui.Name = 'Zuro✿PerformanceMetrics'
...
local function create_ball_stats_gui()
    ...
    local OverlayGui = Instance.new('ScreenGui')
    OverlayGui.Name = 'Zuro✿BallMetrics'
    OverlayGui.ResetOnSpawn = false
    OverlayGui.IgnoreGuiInset = true
    OverlayGui.DisplayOrder = 99
    OverlayGui.Parent = CoreGui
...
getgenv()._Zuro✿_ESP_Stop = stop_ability_esp
...
local function cleanup_visual_stats()
    ...
    destroy_ball_stats()
    pcall(function()
        if getgenv()._Zuro✿_PlayerPanel_Stop then getgenv()._Zuro✿_PlayerPanel_Stop() end
    end)
end
...
do
    ...
    getgenv()._Zuro✿_PlayerPanel_Stop = destroy_player_panel
end
...
do
	...
	getgenv()._Zuro✿_TB_Stop = stop_triggerbot
	getgenv()._Zuro✿_TB_Set = function(state)
		...
		if getgenv()._Zuro✿_Triggerbot_UI_Update then
			getgenv()._Zuro✿_Triggerbot_UI_Update()
		end
	end
	...
	triggerbot_module:create_checkbox({
		...
		callback = function(value)
			if value then
				if getgenv()._Zuro✿_Triggerbot_UI_Create then
					getgenv()._Zuro✿_Triggerbot_UI_Create()
				end
			else
				if getgenv()._Zuro✿_Triggerbot_UI_Destroy then
					getgenv()._Zuro✿_Triggerbot_UI_Destroy()
				end
			end
		end,
	})
...
do
	...
	local function create_triggerbot_ui()
		...
		ToggleHit.Activated:Connect(function()
			if getgenv()._Zuro✿_TB_Set then
				getgenv()._Zuro✿_TB_Set(not getgenv().Triggerbot)
			end
		end)
	...
	getgenv()._Zuro✿_Triggerbot_UI_Create = create_triggerbot_ui
	getgenv()._Zuro✿_Triggerbot_UI_Destroy = destroy_triggerbot_ui
	getgenv()._Zuro✿_Triggerbot_UI_Update = update_triggerbot_ui
end
...
do
	...
	local function create_curve_overlay()
		...
		local OverlayGui = Instance.new('ScreenGui')
		OverlayGui.Name = 'Zuro✿CurveOverlay'
		OverlayGui.ResetOnSpawn = false
		OverlayGui.IgnoreGuiInset = true
		OverlayGui.DisplayOrder = 100
		OverlayGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
library:removed(function()
    ...
    pcall(function() if getgenv()._Zuro✿_TB_Stop then getgenv()._Zuro✿_TB_Stop() end end)
    pcall(function() if getgenv()._Zuro✿_ESP_Stop then getgenv()._Zuro✿_ESP_Stop() end end)
    pcall(function() if getgenv()._Zuro✿_StaffDet_Stop then getgenv()._Zuro✿_StaffDet_Stop() end end)
    stop_force_stats()
    if Target_Lock_Highlight then
        Target_Lock_Highlight:Destroy()
        Target_Lock_Highlight = nil
    ...
    pcall(function()
        if getgenv()._Zuro✿_AntiLag_Cleanup then
            getgenv()._Zuro✿_AntiLag_Cleanup()
        end
    end)
    if Connections_Manager["Auto Parry"] then
        Connections_Manager["Auto Parry"]:Disconnect()
    ...
    kill_preclick_stop()
    pcall(function()
        if getgenv()._Zuro✿_TD_Stop then getgenv()._Zuro✿_TD_Stop() end
    end)
end)

getgenv()._Zuro✿_Cleanup = function()
